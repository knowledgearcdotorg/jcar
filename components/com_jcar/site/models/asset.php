 ...
 */
class JCarModelAsset extends JModelItem
{
    protected $item;
    ...
    {
        $pk = (!empty($pk)) ? $pk : $this->getState('asset.id');
        ...
        if (!JArrayHelper::getValue($this->item, $pk)) {
            ...
            try {
                $event = 'onJCarAssetRetrieve';
                $vars = array($pk);

                // Trigger the data preparation event.
                $responses = $dispatcher->trigger($event, $vars);
            } catch (Exception $e) {
            ...
            if (!$valid) {
                throw new Exception("The asset does not exist.", 404);
            }
        }
